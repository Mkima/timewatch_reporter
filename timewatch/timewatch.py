...
class TimeWatch:
    def __init__(self, loglevel=logging.WARNING, **kwargs):
        ...
        self.taskreport = "projects/po_tasks.php"
    ...
    def login(self, company, user, password):
        ...
        r = self.session.post('https://c.timewatch.co.il/user/validate_user.php', data, self.headers )
        #if not ( "בוקר טוב") in r.text: # if "The login details you entered are incorrect!" in r.text:
    ...
    def task_report(self):
        ...
        dt = BeautifulSoup(rp_data.text)
        return rp_data.text

    def parse_table(self,html_data, html_file):
        ...
        for d in df_data:
            ...
            for i, r in d.iterrows():
                if not relevant_table and type(r) is pd.Series and type(r[0]) is str and 'תאריך' == r[0]:
                    header=r.tolist()
                    relevant_table = True
...
if __name__ == "__main__":
  ...
  rp_data = tw.task_report()
  tw.parse_table(rp_data, None)
